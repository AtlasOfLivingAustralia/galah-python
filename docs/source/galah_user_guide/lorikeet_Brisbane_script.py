...
# -------------------------------------------------------------------------------------------------------
plot_brisbane_parks_bbox = shapely.box(
    xmin=brisbane_parks_bbox["minx"][0],
    xmax=brisbane_parks_bbox["maxx"][0],
    ymin=brisbane_parks_bbox["miny"][0],
    ymax=brisbane_parks_bbox["maxy"][0],
)
brisbane_parks_all.plot(
    edgecolor="#5A5A5A", linewidth=1, facecolor="white", figsize=(7, 10)
)
plt.plot(*plot_brisbane_parks_bbox.exterior.xy, color="red")
plt.ylabel("Latitude", size=16, x=0.45, y=0.5)
...
plt.xlabel("Longitude", size=16)
plt.scatter(
    lorikeet_brisbane["decimalLongitude"],
    lorikeet_brisbane["decimalLatitude"],
    alpha=0.5,
    color="orange",
    label="Lorikeet occurrences",
)
...
for i, park in brisbane_parks.iterrows():
    points = [
        (x, y)
        for x, y in zip(
            lorikeet_brisbane["decimalLongitude"], lorikeet_brisbane["decimalLatitude"]
        )
        if shapely.contains_xy(park["geometry"], x, y)
    ]
    brisbane_parks.at[i, "count"] = len(points)

brisbane_parks_counts = (
    brisbane_parks[["PARK_NAME", "count"]]
